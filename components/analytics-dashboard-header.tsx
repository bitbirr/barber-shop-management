...
export function AnalyticsDashboardHeader() {
  ...
  function exportCsv() {
    ...
    anchor.href = url;
    anchor.download = "faded-analytics.csv";
    anchor.click();
    ...
    <header className="flex flex-col gap-6 border-b border-slate-200/80 pb-6 dark:border-white/[0.08] 2xl:flex-row 2xl:items-end 2xl:justify-between">
      <div>
        <p className="mb-1 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.14em] text-emerald-700 dark:text-emerald-300">
          <span className="size-1.5 rounded-full bg-emerald-500" /> Analytics
        </p>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end 2xl:justify-end">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="block min-w-[176px]">
            ...
            <span className="relative block">
              ...
              <select
                className="h-10 w-full appearance-none rounded-xl border border-slate-200 bg-white pl-9 pr-9 text-sm font-semibold text-slate-700 shadow-sm outline-none transition hover:border-slate-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/15 dark:border-white/10 dark:bg-white/[0.04] dark:text-white/75"
          ...
          <div>
            ...
            <button
              ...
              className="flex h-10 items-center gap-3 whitespace-nowrap rounded-xl border border-slate-200 bg-white px-3.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 dark:border-white/10 dark:bg-white/[0.04] dark:text-white/75"
              onClick={() => setComparisonEnabled((current) => !current)}
              role="switch"
              type="button"
            >
              <span className={`relative h-5 w-9 rounded-full transition ${comparisonEnabled ? "bg-emerald-500" : "bg-slate-200 dark:bg-white/15"}`}>
        ...
        <div className="flex items-center gap-2">
          <button className="inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 dark:border-white/10 dark:bg-white/[0.04] dark:text-white/70 dark:hover:bg-white/[0.08] sm:flex-none" onClick={exportCsv} type="button">
          ...
          <div className="relative flex-1 sm:flex-none" ref={shareMenuRef}>
            <button
              ...
              aria-haspopup="menu"
              className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2 dark:bg-emerald-400 dark:text-[#101815] dark:hover:bg-emerald-300"
            ...
            {shareOpen && (
              <div className="absolute right-0 z-30 mt-2 w-72 rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_16px_45px_rgba(15,23,42,0.14)] dark:border-white/10 dark:bg-[#18211e]" role="menu">
                ...
                <button className="flex w-full items-center gap-3 rounded-xl px-2.5 py-2.5 text-left text-sm font-medium text-slate-700 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 dark:text-white/70 dark:hover:bg-white/[0.06]" onClick={copyShareLink} role="menuitem" type="button">
                  <span className="grid size-8 place-items-center rounded-lg bg-slate-100 text-slate-500 dark:bg-white/[0.07] dark:text-white/50">{copied ? <Check aria-hidden="true" className="size-4 text-emerald-600" /> : <Copy aria-hidden="true" className="size-4" />}</span>
                  <span>{copied ? "Link copied" : "Copy share link"}</span>
                </button>
                <a className="flex items-center gap-3 rounded-xl px-2.5 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 dark:text-white/70 dark:hover:bg-white/[0.06]" href="mailto:?subject=Faded analytics report" role="menuitem">
